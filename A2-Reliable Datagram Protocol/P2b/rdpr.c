...
void writedata()
{
    ...
    memset(tmp, 0, MAX_PAYLOAD+1);
    for (i = 0; i < numBufferSegments; i++) {
        memcpy(tmp, buffer + i * MAX_PAYLOAD, MAX_PAYLOAD);
        tmp[MAX_PAYLOAD] = '\0';
        fprintf(f, "%s", tmp);
    }
}
...
void parsesegment()
{
    ...
    if (current_state == transfer)
    {
        ...
        if (resultsegment.seqno == 1) {
            resultsegment.seqno = 0;
        }
        ...
        { //FIN !! Finally
            ...
            printf("total data bytes received: %d\nunique data bytes received: %d\ntotal data segments received: %d\nunique data segments received: %d\nSYN segments received: %d\nFIN segments received: %d\nRST segments received: %d\nACK segments sent: %d\nRST segments sent: %d\ntotal time duration (second): %f\n", summaryInfo.totalbytesend, summaryInfo.uniquedata, summaryInfo.totalsegment, summaryInfo.uniquesegment, summaryInfo.syn, summaryInfo.fin, summaryInfo.rstsend, summaryInfo.ack, summaryInfo.rstrecieve, summaryInfo.totaltimeduration);
            fclose(f);
            exit(-1);
        }

        summaryInfo.totalbytesend += resultsegment.length;
        ...
        char datacontent[resultsegment.length+1]; //temp buffer for data
        ...
        if (!validBuffer[thisBufferIndex]) {
            summaryInfo.uniquedata += resultsegment.length;
            summaryInfo.uniquesegment++;

        }
        validBuffer[thisBufferIndex] = 1;
        memcpy(buffer+thisBufferIndex * MAX_PAYLOAD, datacontent, resultsegment.length);
